            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container>
                        ...
                        <v-text-field v-model="formCek.keluhan" :rules="nameRules" label="Keluhan"
                            required></v-text-field>
                    </v-container>
                </v-card-text>
                <v-card-actions>
                    ...
                    <v-btn color="blue darken-1" text @click="setContact"> Save </v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
export default {
    ...
    methods: {
        ...
        getData() {
            var url = this.$api + '/kontakkami';
            this.$http.get(url, {
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('token')
                }
            }).then(response => {
                this.cekContacts = response.data.data;
            })
        },


        tambah() {
            this.cekContact.append('kodePengiriman', this.formCek.kodePengiriman);
            this.cekContact.append('keluhan', this.formCek.keluhan);

            var url = this.$api + '/kontakkami'
            this.load = true;
            this.$http.post(url, this.cekContact, {
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('token'),
                }
            }).then(response => {
        ...
        edit() {
            let newData = {
                ...
                keluhan: this.formCek.keluhan,
            };
            var url = this.$api + '/kontakkami/' + this.editIndex;
            this.load = true;
        ...
        deleteData() {
            var url = this.$api + '/kontakkami/' + this.deleteIndex;
            this.$http.delete(url, {
                ...
                .then((response) => {
                    ...
                    this.snackbar = true;
                    this.load = false;
                    this.cekContacts.splice(this.index, 1);
                    this.close();
                    this.getData();
                    this.resetForm();
                    this.inputan = "tambah";
        ...
        editItem(item) {
            ...
            this.formCek.keluhan = item.keluhan;
            this.dialog = true;
        },
