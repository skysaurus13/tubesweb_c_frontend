  ...
  <v-main class="list-profil">
    <v-card>
      <v-list-item>
        ...
        <v-list-item-content>
          <v-list-item-title class="headline" style="font-weight: bold;">Edit Profil User</v-list-item-title>
        </v-list-item-content>
        <v-card-title>
          <v-btn color="success" dark :to="{ name: 'Profil' }">Profil</v-btn>
        </v-card-title>
      </v-list-item>
    </v-card>
    <v-card>
      <v-card-text>
        ...
        <v-text-field :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'" :type="show1 ? 'text' : 'password'"
          :rules="passwordRules" label="Password" v-model="user.password" required
          @click:append="show1 = !show1"></v-text-field>
        <v-text-field label="Nomor Telepon" v-model="user.nomorTelepon" :rules="noTlpRules"
          required>{{ user.nomorTelepon }}</v-text-field>
        <v-spacer></v-spacer>
  ...
  methods: {
    ...
    updateData() {
      let dataProfile = {
        ...
        nomorTelepon: this.user.nomorTelepon,
      };
      this.$http.put(this.$api + '/user/' + localStorage.getItem('id'), dataProfile, {
        headers: {
          'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
      }).then(() => {
